	...
	if (!io_service)
	{
		...
		if (!io_service)
		{
			NSRunCriticalAlertPanel( 
								NSLocalizedString( @"ALPS or Synaptics not found", "MsgBox"), 
...
- (void)didUnselect
{
	...
	f=fopen ([[NSHomeDirectory() stringByAppendingString:[NSString stringWithUTF8String: "/Library/Preferences/org.voodoo.Touchpad.plist"]] UTF8String], "wb");
	if (!f)
	{
		NSRunCriticalAlertPanel( 
								NSLocalizedString( @"Couldn't save plist", "MsgBox"), 
